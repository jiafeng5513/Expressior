...
def extract_data(filename, num_images):
  ...
  with gzip.open(filename) as bytestream:
    ...
    #print('sharp', num_images, IMAGE_SIZE, IMAGE_SIZE, 1)
    return data
...
def get_data(data_root):
  ...
  print("V集合",len(validation_data),len(validation_labels))
  train_data = train_data[VALIDATION_SIZE:, :, :, :]
  train_labels = train_labels[VALIDATION_SIZE:]
  print("t集合",len(train_data),len(train_labels))
...
def _activation_summary(x):
  ...
  # 上一行报错:module 'tensorflow' has no attribute 'histogram_summary'改为下面的样子:
...
def build_model():
  ...
  #上面这个函数的调用方法已经改了,改成下面这样:
  loss =tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=train_labels_node))
  ...
  #summaries=tf.merge_all_summaries()弃用,改为下面
...
def main():
  ...
  train_data, train_labels, validation_data, validation_labels, test_data, test_labels = get_data("./input_data/")
  #上一行报错:TypeError: get_data() missing 1 required positional argument: 'data_root'
  #这是个自定义函数,参数的含义是装mnist数据的四个压缩包的文件路径

  #pdb.set_trace()

  # build net (return inputs)
  #返回的变量数量多于原来用于接受值的变量,按名补全
  (train_data_node,
  train_labels_node,
  validation_data_node,
  test_data_node,
  ...
  with tf.Session() as s:
    ...
    print('-----summary输出目录绑定-----')
    #for step in xrange(NUM_EPOCHS * TRAIN_SIZE // BATCH_SIZE):弃用方法xrange
    print('-----循环次数---->',NUM_EPOCHS * TRAIN_SIZE // BATCH_SIZE)
